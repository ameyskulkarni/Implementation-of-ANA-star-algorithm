...
def ana_star_code(start_node, exit_node):
    ...
    while len(openl)!=0:
        ...
        for i in range(0,len(grid)):
            if i!=pos:
                if ((grid[i].x == grid[pos].x+1 and grid[i].y == grid[pos].y+1) or (grid[i].x == grid[pos].x-1 and grid[i].y == grid[pos].y-1) or (grid[i].x == grid[pos].x+1 and grid[i].y == grid[pos].y-1) or (grid[i].x == grid[pos].x-1 and grid[i].y == grid[pos].y+1)):
                    if grid[i].color!="1":
                        succ.append(grid[i])
        ...
        for i in range (0,len(succ)):
            if curr_node.g+1.414<succ[i].g:
                succ[i].g=curr_node.g+1.414
                succ[i].parent=curr_node
